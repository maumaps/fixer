...
fn patch_explanation_quality_failure(
    ...
    if let Some(error) =
        evidence_confidence_quality_failure(&response, &issue_connection, &validation)
    {
        return Some(error);
    }
...
fn evidence_confidence_quality_failure(
    response: &str,
    issue_connection: &str,
    validation: &str,
) -> Option<String> {
    ...
    let has_source_changes = response_lists_source_changes(response);
    if has_source_changes
        && confidence != "reproduced"
        && response_has_security_sensitive_patch_surface(response, issue_connection)
    {
    ...
    if confidence == "inferred" && has_source_changes {
...
fn response_lists_source_changes(response: &str) -> bool {
    extract_git_add_paths_from_response(response)
        .into_iter()
        .map(|path| {
            path.trim()
                .trim_matches('`')
        ...
        .any(|path| {
            !path.is_empty() && !matches!(path.as_str(), "none" | "(none)" | "no changes" | "n/a")
        })
}
...
"#;
    ...
    fn upstream_prompts_warn_against_invented_reproducers() {
        ...
        assert!(plan_prompt.contains("inferred source patches are blocked"));
        assert!(plan_prompt.contains("## Evidence Confidence"));
        assert!(review_prompt.contains("Reject patches that invent a reproducer"));
        assert!(review_prompt.contains("claim `reproduced` unless `## Validation`"));
        ...
        assert!(
            super::patch_response_contract()
                .contains("include that command/test in `## Validation`")
        );
        assert!(
            super::patch_response_contract()
                .contains("inferred source patches are not pull-request-ready")
        );
        assert!(
            super::patch_response_contract()
                .contains("observed by Fixer and not independently reproduced")
        );
...
## Issue Connection
Users reported that layout switching picked the wrong slot after a spare-layout change, and the failure was observed by Fixer and not independently reproduced. The patch updates the runtime XKB list and the logical layout path so the issue is fixed.
...
"#;
    ...
    #[test]
    fn patch_explanation_quality_guard_rejects_observed_auth_timing_patch() {
        let response = r#"Subject: auth2: avoid delay amplification after slow auth

## Commit Message
Return immediately when an authentication attempt already took longer than the requested failed-auth delay.

## Evidence Confidence
observed

## Issue Connection
Fixer observed sampled sshd-auth CPU in the failed-authentication timing path, and I did not independently reproduce the trace. This patch changes authentication delay handling so the expected effect is avoiding extra sleep after slow authentication work.

## Git Add Paths
auth2.c

## Validation
Ran `autoreconf && ./configure && make -j2 auth2.o sshd-auth`.
"#;
...
"#;
    ...
    #[test]
    fn patch_explanation_quality_guard_accepts_observed_auth_no_patch_report() {
        let response = r#"Subject: No patch for failed-auth delay timing

## Commit Message
No source change was made because the collected OpenSSH failed-auth timing signal touches authentication throttling behavior and needs security-sensitive review.

## Evidence Confidence
observed

## Issue Connection
Fixer observed sampled sshd-auth CPU in the failed-authentication timing path, and I did not independently reproduce the trace. This is a no-patch diagnosis/report rather than a source patch, so the expected effect is avoiding a speculative authentication timing change until the behavior is reproduced and reviewed for security impact.

## Git Add Paths
None

## Validation
Ran `autoreconf && ./configure && make -j2 auth2.o sshd-auth`; reproduction of the timing behavior was not completed.
"#;
...
"#;
    ...
    #[test]
    fn patch_explanation_quality_guard_requires_reproduced_validation_evidence() {
        let response = r#"Subject: server: throttle cron RSS sampling

## Commit Message
Reduce periodic RSS sampling during idle cron ticks.

## Evidence Confidence
reproduced

## Issue Connection
I reproduced the idle Redis `/proc/self/stat` polling locally and confirmed that the patch reduces it.
...
"#;
        ...
        assert!(error.contains("reproduction command/test"));
    }

    #[test]
    fn patch_explanation_quality_guard_accepts_reproduced_with_validation_evidence() {
        let response = r#"Subject: server: throttle cron RSS sampling
...
## Issue Connection
I reproduced the idle Redis `/proc/self/stat` polling loop locally with a strace smoke check and confirmed that the patch changes the cron sampling cadence. The expected effect is fewer procfs reads while the server is idle.
...
"#;
    ...
    #[test]
    fn patch_explanation_quality_guard_accepts_inferred_no_patch_diagnosis() {
        let response = r#"Subject: No patch for idle postmaster wait

## Commit Message
No source change was made because the collected signal points at an idle wait path.

## Evidence Confidence
inferred

## Issue Connection
The profiler signal suggests a wait-loop sample, but the bundle does not contain direct evidence of a user-visible failure. This is a no-patch diagnosis rather than a source patch, so the expected effect is avoiding a speculative change until stronger evidence is gathered.

## Git Add Paths
None

## Validation
Ran configure and build checks.
"#;
    ...
    #[test]
    fn patch_explanation_quality_guard_requires_observed_patch_reproduction_caveat() {
        let response = r#"Subject: server: throttle cron RSS sampling
...
## Issue Connection
Fixer observed a Redis trace repeatedly opening `/proc/self/stat`. This patch throttles the cron sampling path, so the expected effect is fewer idle proc reads.
...
"#;
        ...
        assert!(error.contains("not independently reproduced"));
    }
